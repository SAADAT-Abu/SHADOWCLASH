...
## D-016 — Internet play: rendezvous server + UDP hole punching (room tokens)
**Date:** 2026-07-20
**What:** Implemented ONLINE_PLAY.md Option 3. A stdlib-only rendezvous server (`shadowclash/network/rendezvous_server.py`) runs on the user's VPS (65.20.110.24:5556/udp, systemd unit `shadowclash-rendezvous`, DynamicUser, ufw opened). Hosts auto-register over the *game* socket (so the NAT mapping matches) and get a 6-char room token, shown on the WAITING screen next to the LAN IP. The join input auto-detects: dotted-quad = direct IP (LAN path unchanged), anything else = token via the server. On JOIN both peers receive each other's public *and* private endpoints and fire game packets at all candidates (`UdpSender.punch_targets`) until real packets arrive, then lock onto the working address — the private candidate covers same-NAT players where hairpin routing fails. Rooms expire after 5 min without keepalive; keepalives piggyback until peers connect, then the 30Hz game stream holds the NAT open. The server never relays game traffic (a relay fallback for symmetric NATs remains future work).
**Why:** User provisioned a cloud server and asked for cross-network play with a single input field for IP-or-token.

## D-017 — Container run flags: no /dev/video0 bind, /run/user bind for X auth + audio
**Date:** 2026-07-20
**What:** The spec's documented `singularity run` command was wrong on real desktops in three ways. (a) `--bind /dev/video0` actively *breaks* the camera: Singularity mounts the whole host `/dev` by default (`mount dev = yes`), and adding a single-file device bind shadows the node with a mount the kernel refuses device access through — `open()` fails with EACCES even though the host ACL grants the user rw. Removing the bind fixes it; the default `/dev` mount carries the host ACLs and works. (b) Modern desktops (Fedora/GNOME on Wayland at least) require the X auth cookie; without it XWayland rejects the connection ("Authorization required, but no authorization protocol specified"). Fix: `--bind /run/user/$(id -u)` + `--env XAUTHORITY=$XAUTHORITY`. (c) The same `/run/user` bind exposes the PulseAudio/PipeWire socket, fixing "Failed to create secure directory (/run/user/1000/pulse)". `scripts/run_container.sh` assembles these flags conditionally (skips missing dirs/vars) so one command works across distros; `%help`, `%runscript`, and CLAUDE.md §6 were updated to match.
**Why:** First real container run failed on the build machine itself with the spec's command; the portable launcher is what makes the single-file `.sif` actually runnable on any PC.

## D-018 — Windows/macOS distribution via PyInstaller + GitHub Actions
**Date:** 2026-07-20
**What:** A Singularity `.sif` is Linux-only, but the user wants the game on Windows and Mac too. The game code is already cross-platform (pure Python; mediapipe/opencv/pygame/pymunk all ship Windows and macOS wheels; fonts use pygame's built-in default; sound degrades silently), so no code changes were needed — only packaging. Added `shadowclash.spec` (PyInstaller one-dir bundle; `collect_all("mediapipe")` pulls in the .tflite models; `assets/` and `config/` land at the bundle root, which is exactly where the game's `Path(__file__).parents[2]` lookups resolve when frozen) and `.github/workflows/build-desktop.yml` (matrix: windows-x64, macos-arm64, macos-intel, linux-x64; runs on demand or on `v*` tags, smoke-tests each bundle with SDL dummy drivers, uploads zips, attaches them to the release on tag builds). Validated locally: the Linux bundle (~830MB unpacked) builds from the same spec, finds its bundled config/assets, and runs the menu headless without errors. Native binaries cannot be cross-compiled from Linux, so the Windows/Mac builds require pushing the repo to GitHub and running the workflow — there is no local-only path to them.
**Why:** Cross-platform reach without maintaining three build machines; the container stays the Linux distribution channel (D-017), PyInstaller zips cover Windows/Mac.

## D-019 — VS match phases, best-of-N rounds, protocol v4
**Date:** 2026-07-20
**What:** User bug batch. (a) The multiplayer loop is now a phase machine: waiting -> countdown -> fighting -> round_over -> (countdown ...) -> match_over. The 99s fight timer previously started at launch, so a host waiting >99s hit "TIME UP" and the KO overlay stacked on the WAITING overlay — that was both the "gibberish text" report and the perceived "token dies too fast" (the room itself lives 5 min server-side with 10s keepalives; nothing needed changing there — the waiting room now lasts indefinitely). Exactly one center overlay renders per phase. (b) The fight starts `round_gap_seconds` (30) after the peer's stream appears, with an on-screen ROUND N countdown, beeps on the last 5 seconds (new count.wav) and the bell + FIGHT! flash at zero. (c) Best-of-N rounds (host picks 3/5/7 via a new settings row; `RoundTracker` in modes/rounds.py is pure/testable): 30s gap between rounds, HP resets each round, Tekken-style round pips under the health bars and a ROUND n/N banner; match ends at majority wins or when rounds run out (draw rounds score nobody). Protocol bumped to v4 with a rounds byte in the header so the joiner adopts the host's setting — both clients then derive identical round transitions from the shared damage numbers. (d) KO sound rebuilt (52Hz sub-boom + gong + crack, 3.2s) and now fires at every round end and match end. (e) `damage.vs_scale: 0.6` softens every VS-mode hit (13.5 max head instead of 22.5) so fights stop ending in ~20s; single-player ladder and training are untouched. Verified with a loopback bot match: both clients logged identical round timelines and mirror-consistent scores, hit counts symmetric (12/17 vs 17/12).
**Why:** Direct user playtest feedback after hosting real internet matches.

## D-020 — Relay fallback for symmetric NAT; joining is token-only
**Date:** 2026-07-21
**What:** A real internet match between two players behind the same carrier-grade NAT never connected, despite the rendezvous working perfectly (token issued, peers introduced, both punching). A STUN probe from the host explained why: the same local socket was mapped to `84.88.183.253:5598` when talking to Google and `84.88.183.253:12954` when talking to Cloudflare — **symmetric NAT**, where the public port varies per destination. The endpoint the rendezvous observes is therefore not one the peer can ever reach, so hole punching is impossible by construction, not by misconfiguration (D-016 flagged this fallback as future work). Both players also shared one public IP on isolated private subnets (10.250.10.x and 172.16.43.x), so the private candidates were unroutable too, and joining by LAN IP could not work either. Fix: the rendezvous server now also relays. New `SHRV1 RLY <token> host|join <local>` registers a side; once both are registered, any non-SHRV1 datagram from one member is forwarded verbatim to the other. Clients punch first and add the server as an extra destination after `network.relay_after_seconds` (6) of silence — both paths stay live and the receiver's existing seq filter discards the duplicates, so a direct path still wins when one exists. The client names its own side because the server cannot infer it: a NAT rebind mid-match just looks like a new address, and guessing evicts the wrong half of the pair (caught by a test). Relay keepalives run for the whole match, holding the room, the pair and the NAT mapping open. Server buffer raised 512 -> 2048 bytes (a game packet is 1233) and relayed packets are never logged. Cost is ~75 KB/s per active match. Also: joining is now **token-only** in the menu — the token path already covers LAN (it punches the private candidate and connects directly, no relay), so the IP box was redundant and had misled the user into typing an unroutable address; `--ip` survives as a CLI flag for offline LAN parties and loopback tests. Two silent failures were surfaced: unreadable packets (a version mismatch, previously dropped with zero output) now log once and show "VERSION MISMATCH", and the waiting screen reports the actual stage (PEER FOUND / connecting via relay) instead of a static WAITING. Verified with a rendezvous stub that hands out unroutable TEST-NET-1 candidates, forcing the relay: a full best-of-3 completed with identical round timelines and mirrored scores on both clients; the deployed VPS server then relayed real 1233-byte packets intact in both directions.
**Why:** Symmetric NAT is common on mobile and campus networks, and it is the one case no amount of punching can fix. Deployed to 65.20.110.24 (old file kept as `.bak-20260721`); the change is backward compatible, since HOST/KEEP/JOIN/BYE behave exactly as before for older clients.
